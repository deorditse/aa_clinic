  ...
  // функция вызывает fetchData только при достижении определенного интервала (чтобы не перегружать)
    Future safeFetchData({
      ...
      if(lastDataFetchedDateTime == null ||
          lastDataFetchedDateTime!.isBefore(DateTime.now().subtract(const Duration(minutes: 1)) )){
    ...
    Future fetchData({
        ...
        Map<String, dynamic>? lastValues = await getLastValues(
        ...
        if (socketIOAvailable) {
            socket?.emit('createHealthSnapshotFromArray', { "data": _healthDataList});
        } else {
    ...
    Future<ActiveValue?> getActiveHealthValues() async {
        ...
        if (health.isDataTypeAvailable(HealthDataType.HEART_RATE_VARIABILITY_SDNN)) {
        ...
        if (requested) {
            try {
                if (health.isDataTypeAvailable(HealthDataType.HEART_RATE_VARIABILITY_SDNN)) {
                    arrhythmia = await health.getHealthDataFromTypes(prev_date, now, [HealthDataType.HEART_RATE_VARIABILITY_SDNN]);
                }
                oxygen = await health.getHealthDataFromTypes(prev_date, now, [HealthDataType.BLOOD_OXYGEN]);
                heartRate = await health.getHealthDataFromTypes(prev_date, now, [HealthDataType.HEART_RATE]);
                sleepinbed = await health.getHealthDataFromTypes(prev_date, now, [HealthDataType.SLEEP_IN_BED]);
                energyBurned = await health.getHealthDataFromTypes(prev_date, now, [HealthDataType.ACTIVE_ENERGY_BURNED]);
